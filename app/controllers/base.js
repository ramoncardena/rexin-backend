...
exports.sendContact = async (data, callback) => {
  const auth = {
    auth: {
  ...
  const mailOptions = {
    from: `${data.user.name} <${data.user.email}>`,
    to: `${process.env.CONTACT_FORM_TO_NAME} <${
      process.env.CONTACT_FORM_TO_ADDRESS
    }>`,
...
exports.sendResetPasswordEmailMessage = async user => {
  ...
  const htmlMessage = `<p>To recover the password for user: ${
    user.email
  }</p> <p>click the following link:</p> <p>${
    process.env.FRONTEND_URL
  }/pwreset/${
    user.verification
